...
color_encode = {'red': [1, 0, 0, 0], 'green': [0, 1, 0, 0], 'blue': [0, 0, 1, 0], 'magenta': [1, 0, 1, 0], 'none': [0, 0, 0, 0]}
...
def convert_to_image(text_file, save_as='result', size=1, color='none'):
    ...
    if os.path.isfile(text_file) and text_file[-4:] == '.txt':
        ...
        text = text_file
    color_list = []
    # Inserting null characters to make the string length a multiple of four
    if len(text) % 4:
        text += '\00' * (4-len(text) % 4)
    ...
    # Grouping every 4 item in a tuple
    color_list = [tuple(color_list[i:i+4]) for i in range(0, len(color_list), 4)]
    ...
    img = Image.new('RGBA', (largest_square*size, height*size), color=(0, 0, 0, 0))
    pixels = img.load()
